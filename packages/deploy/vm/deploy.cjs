...
const scriptPath = path.join(deployDir, 'deploy.sh');
...
if (isWindows) {
  ...
  if (!hasWSL) {
    console.error(
      '\n❌ Error: Windows Subsystem for Linux (WSL) is required for deployment!',
    );
    console.error(
      '\nAnsible (the deployment tool) does not run natively on Windows.',
    );
    console.error('You need to install WSL to deploy from Windows:');
    console.error(
    ...
    console.error(
      '\nAlternatively, you can deploy from a Mac/Linux machine or CI/CD pipeline.',
    );
    process.exit(1);
  }
  ...
  console.log('🚀 Running deployment through WSL...\n');

  // Use execSync with cd command to avoid path translation issues
  // Git Bash translates /mnt/ paths, so we cd in WSL first
  try {
    const wslCommand = `wsl.exe bash -c "cd ${wslDeployDir} && bash ./deploy.sh ${args.join(' ')}"`;
    execSync(wslCommand, {
      stdio: 'inherit',
    });
    process.exit(0);
