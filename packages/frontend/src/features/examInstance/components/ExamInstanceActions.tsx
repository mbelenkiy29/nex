...
export function ExamInstanceActions({
  mode,
  examInstance,
  referrer,
}: {
  mode: 'table' | 'view';
  examInstance: ExamInstanceWithRelationships;
  referrer?: string;
}) {
  ...
  const deleteMutation = useMutation({
    mutationFn: async () => {
      return await apiClient
        .delete(
          `api/exam-instance?${objectToQuery({ ids: [examInstance.id] })}`,
        )
        .json();
    },
    onSuccess: () => {
      ...
      toast.success(dictionary.examInstance.delete.success);
    },
    onError: (error: any) => {
  ...
  const archiveMutation = useMutation({
    mutationFn: async () => {
      return await apiClient
        .put('api/exam-instance/archive', { json: { ids: [examInstance.id] } })
        .json();
    },
    onSuccess: () => {
      queryClient.resetQueries({
      ...
      toast.success(dictionary.examInstance.archive.success);
    },
    onError: (error: any) => {
      toast.error(error.message || dictionary.shared.errors.unknown);
    },
  ...
  const restoreMutation = useMutation({
    mutationFn: async () => {
      return await apiClient
        .put('api/exam-instance/restore', { json: { ids: [examInstance.id] } })
        .json();
    },
    onSuccess: () => {
      queryClient.resetQueries({
      ...
      toast.success(dictionary.examInstance.restore.success);
    },
    onError: (error: any) => {
      toast.error(error.message || dictionary.shared.errors.unknown);
    },
    ...
    <div className="flex justify-end gap-2">
      <DropdownMenu>
        ...
        <DropdownMenuContent align="end" className="w-[160px]">
          ...
          {hasPermissionToArchive && !examInstance?.archivedAt && (
            <DropdownMenuItem
          ...
          {hasPermissionToRestore && Boolean(examInstance?.archivedAt) && (
            <DropdownMenuItem
      ...
      {archiveDialogOpen && (
        <ConfirmDialog
          title={dictionary.examInstance.archive.confirmTitle}
      ...
      {restoreDialogOpen && (
        <ConfirmDialog
          title={dictionary.examInstance.restore.confirmTitle}
      ...
      {deleteDialogOpen && (
        <ConfirmDialog
          title={dictionary.examInstance.delete.confirmTitle}
