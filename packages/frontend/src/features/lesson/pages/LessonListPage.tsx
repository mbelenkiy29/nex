...
import { LessonListActions } from '@/features/lesson/components/LessonListActions';
import { LessonListFilter } from '@/features/lesson/components/LessonListFilter';
import {
...
import { LessonNewButton } from '@/features/lesson/components/LessonNewButton';
...
const TABLE_ID = 'lesson-list';

export const lessonListLazyRoute = createLazyRoute('/lesson')({
...
export function LessonListPage() {
  ...
  const columns: ColumnDef<LessonWithRelationships>[] = [
    {
    ...
    },
    {
      ...
      meta: {
        title: dictionary.lesson.fields.lessonNumber,
      },
    ...
    },
    {
      ...
      meta: {
        title: dictionary.lesson.fields.estimatedMinutes,
      },
      header: dataTableHeader('right'),
      cell: ({ getValue }) => {
        return (
    ...
    },
    {
      ...
      meta: {
        title: dictionary.lesson.fields.xpReward,
      },
      header: dataTableHeader('right'),
      cell: ({ getValue }) => {
        return (
    ...
    },
    {
      ...
      meta: {
        title: dictionary.lesson.fields.workflowStatus,
      },
      cell: ({ row }) => {
    ...
    },
    {
      ...
      meta: {
        title: dictionary.lesson.fields.isPublished,
      },
      cell: ({ row }) => {
    ...
    },
    {
      accessorKey: 'chapter',
      meta: {
        title: dictionary.lesson.fields.chapter,
      },
      enableSorting: false,
      cell: ({ row }) => {
    ...
    },
    {
      ...
      meta: {
        title: dictionary.lesson.fields.createdByMember,
      },
      enableSorting: false,
      cell: ({ row }) => {
    ...
    },
    {
      ...
      meta: {
        title: dictionary.lesson.fields.createdAt,
      },
      cell: ({ row }) => (
        <span className="whitespace-nowrap">
    ...
    },
    {
      ...
      meta: {
        title: dictionary.lesson.fields.updatedByMember,
      },
      enableSorting: false,
      cell: ({ row }) => {
    ...
    },
    {
      ...
      meta: {
        title: dictionary.lesson.fields.updatedAt,
      },
      cell: ({ row }) => (
        <span className="whitespace-nowrap">
    ...
    },
    {
      ...
      cell: ({ row }) => <LessonActions mode="table" lesson={row.original} />,
      enableSorting: false,
      enableHiding: false,
    },
  ];
  ...
  const query = useQuery({
    queryKey: ['lesson', 'list', filter, sorting, pagination],
    queryFn: async ({ signal }) => {
      return await apiClient
        .get(
          `api/lesson?${objectToQuery({
            filter: filter,
  ...
  const table = useReactTable({
    ...
    data: query.data?.lessons || defaultData,
    columns,
  ...
  return (
    <div className="mb-4 flex w-full max-w-full flex-col gap-4 overflow-hidden p-6">
      <div className="flex items-center justify-between">
        <PageHeader items={[[dictionary.lesson.list.menu]]} />
        {!(
      ...
      {isFilterEmpty(filter) && query.data?.count === 0 && !query.isLoading ? (
        <Empty>
          <EmptyHeader>
            ...
            <EmptyTitle>{dictionary.lesson.list.title}</EmptyTitle>
            <EmptyDescription>{dictionary.lesson.list.empty}</EmptyDescription>
          </EmptyHeader>
          <EmptyContent>
              ...
              <div className="flex gap-2">
                ...
                {hasPermissionToImport && (
                  <Button
                    nativeButton={false}
                    variant="outline"
                    render={<Link to="/lesson/importer" />}
                  >
                    {dictionary.lesson.importer.menu}
                  </Button>
                )}
              </div>
              <Link
        ...
        <>
          <LessonListFilter isLoading={query.isLoading} />

          <DataTable
            table={table}
            isLoading={query.isLoading}
            columns={columns}
            notFoundText={dictionary.lesson.list.noResults}
            newButton={<LessonNewButton />}
          />
