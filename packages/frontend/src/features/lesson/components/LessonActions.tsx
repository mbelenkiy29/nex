...
export function LessonActions({
  ...
  mode: 'table' | 'view';
  lesson: LessonWithRelationships;
  referrer?: string;
}) {
  ...
  const deleteMutation = useMutation({
    mutationFn: async () => {
      return await apiClient
        .delete(`api/lesson?${objectToQuery({ ids: [lesson.id] })}`)
        .json();
    },
    onSuccess: () => {
      ...
      toast.success(dictionary.lesson.delete.success);
    },
    onError: (error: any) => {
  ...
  const archiveMutation = useMutation({
    mutationFn: async () => {
      return await apiClient
        .put('api/lesson/archive', { json: { ids: [lesson.id] } })
        .json();
    },
    onSuccess: () => {
      queryClient.resetQueries({
      ...
      toast.success(dictionary.lesson.archive.success);
    },
    onError: (error: any) => {
      toast.error(error.message || dictionary.shared.errors.unknown);
    },
  ...
  const restoreMutation = useMutation({
    mutationFn: async () => {
      return await apiClient
        .put('api/lesson/restore', { json: { ids: [lesson.id] } })
        .json();
    },
    onSuccess: () => {
      queryClient.resetQueries({
      ...
      toast.success(dictionary.lesson.restore.success);
    },
    onError: (error: any) => {
      toast.error(error.message || dictionary.shared.errors.unknown);
    },
    ...
    <div className="flex justify-end gap-2">
      <DropdownMenu>
        ...
        <DropdownMenuContent align="end" className="w-[160px]">
          ...
          {hasPermissionToArchive && !lesson?.archivedAt && (
            <DropdownMenuItem
          ...
          {hasPermissionToRestore && Boolean(lesson?.archivedAt) && (
            <DropdownMenuItem
      ...
      {archiveDialogOpen && (
        <ConfirmDialog
          title={dictionary.lesson.archive.confirmTitle}
      ...
      {restoreDialogOpen && (
        <ConfirmDialog
          title={dictionary.lesson.restore.confirmTitle}
      ...
      {deleteDialogOpen && (
        <ConfirmDialog
          title={dictionary.lesson.delete.confirmTitle}
