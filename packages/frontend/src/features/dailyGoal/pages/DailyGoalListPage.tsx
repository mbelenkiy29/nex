...
import { DailyGoalListActions } from '@/features/dailyGoal/components/DailyGoalListActions';
import { DailyGoalListFilter } from '@/features/dailyGoal/components/DailyGoalListFilter';
import {
...
import { DailyGoalNewButton } from '@/features/dailyGoal/components/DailyGoalNewButton';
...
const TABLE_ID = 'dailyGoal-list';

export const dailyGoalListLazyRoute = createLazyRoute('/daily-goal')({
  component: DailyGoalListPage,
});

export function DailyGoalListPage() {
  ...
  const columns: ColumnDef<DailyGoalWithRelationships>[] = [
    {
    ...
    },
    {
      ...
      meta: {
        title: dictionary.dailyGoal.fields.targetValue,
      },
    ...
    },
    {
      ...
      meta: {
        title: dictionary.dailyGoal.fields.currentValue,
      },
      header: dataTableHeader('right'),
      cell: ({ getValue }) => {
        return (
    ...
    },
    {
      ...
      meta: {
        title: dictionary.dailyGoal.fields.xpReward,
      },
      header: dataTableHeader('right'),
      cell: ({ getValue }) => {
        return (
    ...
    },
    {
      ...
      meta: {
        title: dictionary.dailyGoal.fields.goalDate,
      },
      cell: ({ row }) => (
        <span className="whitespace-nowrap">
          {formatDate(row.getValue('goalDate'), dictionary)}
        </span>
      ),
    },
    {
      ...
      meta: {
        title: dictionary.dailyGoal.fields.completedAt,
      },
      cell: ({ row }) => (
        <span className="whitespace-nowrap">
    ...
    },
    {
      accessorKey: 'owner',
      meta: {
        title: dictionary.dailyGoal.fields.owner,
      },
      enableSorting: false,
      cell: ({ row }) => {
        return <MemberLink member={row.getValue('owner')} />;
      },
    },
    {
      accessorKey: 'createdByMember',
      meta: {
        title: dictionary.dailyGoal.fields.createdByMember,
      },
      enableSorting: false,
      cell: ({ row }) => {
    ...
    },
    {
      ...
      meta: {
        title: dictionary.dailyGoal.fields.createdAt,
      },
      cell: ({ row }) => (
        <span className="whitespace-nowrap">
    ...
    },
    {
      ...
      meta: {
        title: dictionary.dailyGoal.fields.updatedByMember,
      },
      enableSorting: false,
      cell: ({ row }) => {
    ...
    },
    {
      ...
      meta: {
        title: dictionary.dailyGoal.fields.updatedAt,
      },
      cell: ({ row }) => (
        <span className="whitespace-nowrap">
    ...
    },
    {
      ...
      cell: ({ row }) => (
        <DailyGoalActions mode="table" dailyGoal={row.original} />
      ),
      enableSorting: false,
      enableHiding: false,
    },
  ...
  const query = useQuery({
    queryKey: ['dailyGoal', 'list', filter, sorting, pagination],
  ...
  const table = useReactTable({
    ...
    data: query.data?.dailyGoals || defaultData,
    columns,
  ...
  return (
    <div className="mb-4 flex w-full max-w-full flex-col gap-4 overflow-hidden p-6">
      <div className="flex items-center justify-between">
        <PageHeader items={[[dictionary.dailyGoal.list.menu]]} />
        {!(
      ...
      {isFilterEmpty(filter) && query.data?.count === 0 && !query.isLoading ? (
        <Empty>
          <EmptyHeader>
            ...
            <EmptyTitle>{dictionary.dailyGoal.list.title}</EmptyTitle>
            <EmptyDescription>
              {dictionary.dailyGoal.list.empty}
            </EmptyDescription>
          </EmptyHeader>
          <EmptyContent>
            <div className="flex flex-col items-center gap-6">
              <div className="flex gap-2">
                ...
                {hasPermissionToImport && (
                  ...
                  >
                    {dictionary.dailyGoal.importer.menu}
                  </Button>
                )}
              </div>
            </div>
        ...
        <>
          <DailyGoalListFilter isLoading={query.isLoading} />

          <DataTable
            table={table}
            isLoading={query.isLoading}
            columns={columns}
            notFoundText={dictionary.dailyGoal.list.noResults}
            newButton={<DailyGoalNewButton />}
          />
