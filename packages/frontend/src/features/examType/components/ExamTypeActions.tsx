...
export function ExamTypeActions({
  mode,
  examType,
  referrer,
}: {
  mode: 'table' | 'view';
  examType: ExamTypeWithRelationships;
  referrer?: string;
}) {
  ...
  const deleteMutation = useMutation({
    mutationFn: async () => {
      return await apiClient
        .delete(`api/exam-type?${objectToQuery({ ids: [examType.id] })}`)
        .json();
    },
    onSuccess: () => {
      ...
      toast.success(dictionary.examType.delete.success);
    },
    onError: (error: any) => {
  ...
  const archiveMutation = useMutation({
    mutationFn: async () => {
      return await apiClient
        .put('api/exam-type/archive', { json: { ids: [examType.id] } })
        .json();
    },
    onSuccess: () => {
      queryClient.resetQueries({
      ...
      toast.success(dictionary.examType.archive.success);
    },
    onError: (error: any) => {
      toast.error(error.message || dictionary.shared.errors.unknown);
    },
  ...
  const restoreMutation = useMutation({
    mutationFn: async () => {
      return await apiClient
        .put('api/exam-type/restore', { json: { ids: [examType.id] } })
        .json();
    },
    onSuccess: () => {
      queryClient.resetQueries({
      ...
      toast.success(dictionary.examType.restore.success);
    },
    onError: (error: any) => {
      toast.error(error.message || dictionary.shared.errors.unknown);
    },
    ...
    <div className="flex justify-end gap-2">
      <DropdownMenu>
        ...
        <DropdownMenuContent align="end" className="w-[160px]">
          ...
          {hasPermissionToArchive && !examType?.archivedAt && (
            <DropdownMenuItem
          ...
          {hasPermissionToRestore && Boolean(examType?.archivedAt) && (
            <DropdownMenuItem
      ...
      {archiveDialogOpen && (
        <ConfirmDialog
          title={dictionary.examType.archive.confirmTitle}
      ...
      {restoreDialogOpen && (
        <ConfirmDialog
          title={dictionary.examType.restore.confirmTitle}
      ...
      {deleteDialogOpen && (
        <ConfirmDialog
          title={dictionary.examType.delete.confirmTitle}
