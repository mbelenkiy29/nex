import { examTypeExporterMapper } from '@/features/examType/examTypeExporterMapper';
...
export function ExamTypeListActions({
  table,
  count,
  filter,
  sorting,
}: {
  filter: z.input<typeof examTypeFilterInputSchema>;
  sorting: DataTableSort;
  count?: number;
  table: Table<ExamTypeWithRelationships>;
}) {
  ...
  const isFetching = useIsFetching({ queryKey: ['examType'] });
  ...
  const exportMutation = useMutation({
    mutationFn: async () => {
      return await apiClient
        .get(
          `api/exam-type?${objectToQuery({ filter, orderBy: dataTableSortToPrisma(sorting) })}`,
        )
        .json<{ count: number; examTypes: ExamTypeWithRelationships[] }>();
    },
    onSuccess: (data) => {
      csvExporter(
        examTypeExporterMapper(data.examTypes, { dictionary, locale }),
        dictionary.examType.fields,
        'examTypes',
      );
      toast.success(dictionary.examType.export.success);
    },
    onError: (error: any) => {
  ...
  const deleteMutation = useMutation({
    mutationFn: async () => {
      ...
      if (!ids.length) {
        throw new Error(dictionary.examType.deleteMany.noSelection);
      }

      return await apiClient
        .delete(`api/exam-type?${objectToQuery({ ids })}`)
        .json();
    },
    onSuccess: () => {
      ...
      toast.success(dictionary.examType.deleteMany.success);
    },
    onError: (error: any) => {
      toast.error(error.message || dictionary.shared.errors.unknown);
    },
  ...
  const archiveMutation = useMutation({
    mutationFn: async () => {
      ...
      if (!ids.length) {
        throw new Error(dictionary.examType.archiveMany.noSelection);
      }

      return await apiClient
        .put(`api/exam-type/archive`, { json: { ids } })
        .json();
    },
    onSuccess: () => {
      table.resetRowSelection();
      ...
      toast.success(dictionary.examType.archiveMany.success);
    },
    onError: (error: any) => {
      toast.error(error.message || dictionary.shared.errors.unknown);
    },
  ...
  const restoreMutation = useMutation({
    mutationFn: async () => {
      ...
      if (!ids.length) {
        throw new Error(dictionary.examType.restoreMany.noSelection);
      }

      return await apiClient
        .put(`api/exam-type/restore`, { json: { ids } })
        .json();
    },
    onSuccess: () => {
      table.resetRowSelection();
      ...
      toast.success(dictionary.examType.restoreMany.success);
    },
    onError: (error: any) => {
      toast.error(error.message || dictionary.shared.errors.unknown);
    },
  ...
  return (
    <>
      ...
      {archiveManyDialogOpen && (
        <ConfirmDialog
          title={dictionary.examType.archiveMany.confirmTitle}
          description={dictionaryFormat(
            dictionary.examType.archiveMany.confirmDescription,
            selectedCount,
          )}
      ...
      {restoreManyDialogOpen && (
        <ConfirmDialog
          title={dictionary.examType.restoreMany.confirmTitle}
          description={dictionaryFormat(
            dictionary.examType.restoreMany.confirmDescription,
            selectedCount,
          )}
      ...
      {deleteManyDialogOpen && (
        <ConfirmDialog
          title={dictionary.examType.deleteMany.confirmTitle}
          description={dictionaryFormat(
            dictionary.examType.deleteMany.confirmDescription,
            selectedCount,
          )}
