import { documentUploadExporterMapper } from '@/features/documentUpload/documentUploadExporterMapper';
...
export function DocumentUploadListActions({
  table,
  count,
  filter,
  sorting,
}: {
  filter: z.input<typeof documentUploadFilterInputSchema>;
  sorting: DataTableSort;
  count?: number;
  table: Table<DocumentUploadWithRelationships>;
}) {
  ...
  const isFetching = useIsFetching({ queryKey: ['documentUpload'] });
  ...
  const exportMutation = useMutation({
    mutationFn: async () => {
      return await apiClient
        .get(
          `api/document-upload?${objectToQuery({ filter, orderBy: dataTableSortToPrisma(sorting) })}`,
        )
        .json<{
          count: number;
    ...
    onSuccess: (data) => {
      csvExporter(
        documentUploadExporterMapper(data.documentUploads, {
          dictionary,
          locale,
        }),
        dictionary.documentUpload.fields,
        'documentUploads',
      );
      toast.success(dictionary.documentUpload.export.success);
    },
    onError: (error: any) => {
  ...
  const deleteMutation = useMutation({
    mutationFn: async () => {
      ...
      if (!ids.length) {
        throw new Error(dictionary.documentUpload.deleteMany.noSelection);
      }

      return await apiClient
        .delete(`api/document-upload?${objectToQuery({ ids })}`)
        .json();
    },
    onSuccess: () => {
      ...
      toast.success(dictionary.documentUpload.deleteMany.success);
    },
    onError: (error: any) => {
      toast.error(error.message || dictionary.shared.errors.unknown);
    },
  ...
  const archiveMutation = useMutation({
    mutationFn: async () => {
      ...
      if (!ids.length) {
        throw new Error(dictionary.documentUpload.archiveMany.noSelection);
      }

      return await apiClient
        .put(`api/document-upload/archive`, { json: { ids } })
        .json();
    },
    onSuccess: () => {
      table.resetRowSelection();
      ...
      toast.success(dictionary.documentUpload.archiveMany.success);
    },
    onError: (error: any) => {
      toast.error(error.message || dictionary.shared.errors.unknown);
    },
  ...
  const restoreMutation = useMutation({
    mutationFn: async () => {
      ...
      if (!ids.length) {
        throw new Error(dictionary.documentUpload.restoreMany.noSelection);
      }

      return await apiClient
        .put(`api/document-upload/restore`, { json: { ids } })
        .json();
    },
    onSuccess: () => {
      table.resetRowSelection();
      ...
      toast.success(dictionary.documentUpload.restoreMany.success);
    },
    onError: (error: any) => {
      toast.error(error.message || dictionary.shared.errors.unknown);
    },
  ...
  return (
    <>
      ...
      <Button
        ...
        className="ml-auto flex h-8"
        onClick={() =>
          queryClient.invalidateQueries({ queryKey: ['documentUpload'] })
        }
        disabled={!!isFetching}
      >
        {isFetching ? (
      ...
      {hasPermissionToCreate && (
        <Button
          ...
          render={<Link to={`/document-upload/new`} />}
        >
      ...
      {archiveManyDialogOpen && (
        <ConfirmDialog
          title={dictionary.documentUpload.archiveMany.confirmTitle}
          description={dictionaryFormat(
            dictionary.documentUpload.archiveMany.confirmDescription,
            selectedCount,
          )}
      ...
      {restoreManyDialogOpen && (
        <ConfirmDialog
          title={dictionary.documentUpload.restoreMany.confirmTitle}
          description={dictionaryFormat(
            dictionary.documentUpload.restoreMany.confirmDescription,
            selectedCount,
          )}
      ...
      {deleteManyDialogOpen && (
        <ConfirmDialog
          title={dictionary.documentUpload.deleteMany.confirmTitle}
          description={dictionaryFormat(
            dictionary.documentUpload.deleteMany.confirmDescription,
            selectedCount,
          )}
