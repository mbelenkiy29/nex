...
import { DocumentUploadListActions } from '@/features/documentUpload/components/DocumentUploadListActions';
import { DocumentUploadListFilter } from '@/features/documentUpload/components/DocumentUploadListFilter';
import {
  DocumentUploadWithRelationships,
  documentUploadFilterInputSchema,
...
import { DocumentUploadNewButton } from '@/features/documentUpload/components/DocumentUploadNewButton';
...
const TABLE_ID = 'documentUpload-list';

export const documentUploadListLazyRoute = createLazyRoute('/document-upload')({
  component: DocumentUploadListPage,
});
...
export function DocumentUploadListPage() {
  ...
  const columns: ColumnDef<DocumentUploadWithRelationships>[] = [
    {
    ...
    },
    {
      ...
      meta: {
        title: dictionary.documentUpload.fields.originalFilename,
      },
      cell: ({ row }) => (
        <span className="whitespace-nowrap">
          <Link
            className="text-blue-500 hover:text-blue-400 hover:underline focus:text-blue-400 dark:text-blue-400"
            to={`/document-upload/${row?.original?.id}`}
            search={{
              referrer: window.location.pathname + window.location.search,
            }}
          >
            {documentUploadLabel(row?.original, dictionary, locale)}
          </Link>
        </span>
      ),
    },
    {
      accessorKey: 'status',
      meta: {
        title: dictionary.documentUpload.fields.status,
      },
      cell: ({ row }) => {
    ...
    },
    {
      ...
      meta: {
        title: dictionary.documentUpload.fields.pageCount,
      },
    ...
    },
    {
      accessorKey: 'exam',
      meta: {
        title: dictionary.documentUpload.fields.exam,
      },
      enableSorting: false,
      cell: ({ row }) => {
    ...
    },
    {
      ...
      meta: {
        title: dictionary.documentUpload.fields.uploadedBy,
      },
      enableSorting: false,
      cell: ({ row }) => {
        return <MemberLink member={row.getValue('uploadedBy')} />;
      },
    },
    {
      accessorKey: 'createdByMember',
      meta: {
        title: dictionary.documentUpload.fields.createdByMember,
      },
      enableSorting: false,
      cell: ({ row }) => {
    ...
    },
    {
      ...
      meta: {
        title: dictionary.documentUpload.fields.createdAt,
      },
      cell: ({ row }) => (
        <span className="whitespace-nowrap">
    ...
    },
    {
      ...
      meta: {
        title: dictionary.documentUpload.fields.updatedByMember,
      },
      enableSorting: false,
      cell: ({ row }) => {
    ...
    },
    {
      ...
      meta: {
        title: dictionary.documentUpload.fields.updatedAt,
      },
      cell: ({ row }) => (
        <span className="whitespace-nowrap">
    ...
    },
    {
      ...
      cell: ({ row }) => (
        <DocumentUploadActions mode="table" documentUpload={row.original} />
      ),
      enableSorting: false,
      enableHiding: false,
    },
  ...
  const query = useQuery({
    queryKey: ['documentUpload', 'list', filter, sorting, pagination],
  ...
  const table = useReactTable({
    ...
    data: query.data?.documentUploads || defaultData,
    columns,
  ...
  return (
    <div className="mb-4 flex w-full max-w-full flex-col gap-4 overflow-hidden p-6">
      <div className="flex items-center justify-between">
        <PageHeader items={[[dictionary.documentUpload.list.menu]]} />
        {!(
      ...
      {isFilterEmpty(filter) && query.data?.count === 0 && !query.isLoading ? (
        <Empty>
          <EmptyHeader>
            ...
            <EmptyTitle>{dictionary.documentUpload.list.title}</EmptyTitle>
            <EmptyDescription>
              {dictionary.documentUpload.list.empty}
            </EmptyDescription>
          </EmptyHeader>
          <EmptyContent>
            <div className="flex flex-col items-center gap-6">
              <div className="flex gap-2">
                <DocumentUploadNewButton />
                {hasPermissionToImport && (
                  <Button
                    nativeButton={false}
                    variant="outline"
                    render={<Link to="/document-upload/importer" />}
                  >
                    {dictionary.documentUpload.importer.menu}
                  </Button>
                )}
              </div>
              <Link
        ...
        <>
          <DocumentUploadListFilter isLoading={query.isLoading} />

          <DataTable
            table={table}
            isLoading={query.isLoading}
            columns={columns}
            notFoundText={dictionary.documentUpload.list.noResults}
            newButton={<DocumentUploadNewButton />}
          />
