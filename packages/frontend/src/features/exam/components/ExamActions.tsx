...
export function ExamActions({
  ...
  const deleteMutation = useMutation({
    mutationFn: async () => {
      return await apiClient
        .delete(`api/exam?${objectToQuery({ ids: [exam.id] })}`)
        .json();
    },
    onSuccess: () => {
      ...
      toast.success(dictionary.exam.delete.success);
    },
    onError: (error: any) => {
  ...
  const archiveMutation = useMutation({
    mutationFn: async () => {
      return await apiClient
        .put('api/exam/archive', { json: { ids: [exam.id] } })
        .json();
    },
    onSuccess: () => {
      queryClient.resetQueries({
      ...
      toast.success(dictionary.exam.archive.success);
    },
    onError: (error: any) => {
      toast.error(error.message || dictionary.shared.errors.unknown);
    },
  ...
  const restoreMutation = useMutation({
    mutationFn: async () => {
      return await apiClient
        .put('api/exam/restore', { json: { ids: [exam.id] } })
        .json();
    },
    onSuccess: () => {
      queryClient.resetQueries({
      ...
      toast.success(dictionary.exam.restore.success);
    },
    onError: (error: any) => {
      toast.error(error.message || dictionary.shared.errors.unknown);
    },
    ...
    <div className="flex justify-end gap-2">
      <DropdownMenu>
        ...
        <DropdownMenuContent align="end" className="w-[160px]">
          ...
          {hasPermissionToArchive && !exam?.archivedAt && (
            <DropdownMenuItem
          ...
          {hasPermissionToRestore && Boolean(exam?.archivedAt) && (
            <DropdownMenuItem
      ...
      {archiveDialogOpen && (
        <ConfirmDialog
          title={dictionary.exam.archive.confirmTitle}
      ...
      {restoreDialogOpen && (
        <ConfirmDialog
          title={dictionary.exam.restore.confirmTitle}
      ...
      {deleteDialogOpen && (
        <ConfirmDialog
          title={dictionary.exam.delete.confirmTitle}
