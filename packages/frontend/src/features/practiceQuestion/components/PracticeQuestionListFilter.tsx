...
export function PracticeQuestionListFilter({
  isLoading,
}: {
  isLoading: boolean;
}) {
  ...
  const previewRenders = {
    questionText: {
      label: dictionary.practiceQuestion.fields.questionText,
    },
    difficulty: {
      label: dictionary.practiceQuestion.fields.difficulty,
      render: dataTableFilterRenders(locale, dictionary).enumerator(
        dictionary.practiceQuestion.enumerators.difficulty,
      ),
    },
    category: {
      label: dictionary.practiceQuestion.fields.category,
    },
    isActive: {
      label: dictionary.practiceQuestion.fields.isActive,
      render: dataTableFilterRenders(locale, dictionary).boolean(),
    },
    course: {
      label: dictionary.practiceQuestion.fields.course,
      render: dataTableFilterRenders(locale, dictionary).relationToOne(
        courseLabel,
      ),
    },
    chapter: {
      label: dictionary.practiceQuestion.fields.chapter,
      render: dataTableFilterRenders(locale, dictionary).relationToOne(
        chapterLabel,
      ),
    },
    concepts: {
      label: dictionary.practiceQuestion.fields.concepts,
      render: dataTableFilterRenders(locale, dictionary).relationToMany(
        conceptLabel,
      ),
    },
    createdByMember: {
      label: dictionary.practiceQuestion.fields.createdByMember,
      render: dataTableFilterRenders(locale, dictionary).relationToOne(
        memberLabel,
      ),
    },
    updatedByMember: {
      label: dictionary.practiceQuestion.fields.updatedByMember,
      render: dataTableFilterRenders(locale, dictionary).relationToOne(
        memberLabel,
      ),
    },
    createdAtRange: {
      label: dictionary.practiceQuestion.fields.createdAt,
      render: dataTableFilterRenders(locale, dictionary).dateTimeRange(),
    },
    updatedAtRange: {
      label: dictionary.practiceQuestion.fields.updatedAt,
      render: dataTableFilterRenders(locale, dictionary).dateTimeRange(),
    },
    archived: {
      ...
      <div className={cn(expanded ? 'block' : 'hidden', 'p-4')}>
        ...
        >
          <FieldGroup className="grid grid-cols-1 gap-4 md:grid-cols-2">
            <Field>
              <FieldLabel htmlFor="questionText">
              ...
              <Controller
                ...
                render={({ field, fieldState }) => (
                  <>
                    <Input id="questionText" disabled={isLoading} {...field} />
                    {fieldState.error && (
            ...
            <Field>
              ...
              <Controller
                ...
                render={({ field, fieldState }) => (
                  <>
                    <Input id="category" disabled={isLoading} {...field} />
                    {fieldState.error && (
                      <FieldError>{fieldState.error.message}</FieldError>
                    )}
                  </>
            ...
            <Field>
              <FieldLabel>
                {dictionary.practiceQuestion.fields.course}
              </FieldLabel>
              <Controller
                control={form.control}
                name="course"
