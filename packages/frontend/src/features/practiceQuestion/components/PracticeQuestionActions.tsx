...
export function PracticeQuestionActions({
  ...
  mode: 'table' | 'view';
  practiceQuestion: PracticeQuestionWithRelationships;
  referrer?: string;
}) {
  ...
  const deleteMutation = useMutation({
    mutationFn: async () => {
      return await apiClient
        .delete(
          `api/practice-question?${objectToQuery({ ids: [practiceQuestion.id] })}`,
        )
        .json();
    },
    onSuccess: () => {
      ...
      toast.success(dictionary.practiceQuestion.delete.success);
    },
    onError: (error: any) => {
  ...
  const archiveMutation = useMutation({
    mutationFn: async () => {
      return await apiClient
        .put('api/practice-question/archive', {
          json: { ids: [practiceQuestion.id] },
        })
        .json();
    },
    onSuccess: () => {
      ...
      toast.success(dictionary.practiceQuestion.archive.success);
    },
    onError: (error: any) => {
      toast.error(error.message || dictionary.shared.errors.unknown);
    },
  ...
  const restoreMutation = useMutation({
    mutationFn: async () => {
      return await apiClient
        .put('api/practice-question/restore', {
          json: { ids: [practiceQuestion.id] },
        })
        .json();
    },
    onSuccess: () => {
      ...
      toast.success(dictionary.practiceQuestion.restore.success);
    },
    onError: (error: any) => {
      toast.error(error.message || dictionary.shared.errors.unknown);
    },
    ...
    <div className="flex justify-end gap-2">
      <DropdownMenu>
        ...
        <DropdownMenuContent align="end" className="w-[160px]">
          ...
          {hasPermissionToArchive && !practiceQuestion?.archivedAt && (
            <DropdownMenuItem
          ...
          {hasPermissionToRestore && Boolean(practiceQuestion?.archivedAt) && (
            <DropdownMenuItem
      ...
      {archiveDialogOpen && (
        <ConfirmDialog
          title={dictionary.practiceQuestion.archive.confirmTitle}
      ...
      {restoreDialogOpen && (
        <ConfirmDialog
          title={dictionary.practiceQuestion.restore.confirmTitle}
      ...
      {deleteDialogOpen && (
        <ConfirmDialog
          title={dictionary.practiceQuestion.delete.confirmTitle}
