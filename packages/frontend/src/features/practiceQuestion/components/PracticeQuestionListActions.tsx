import { practiceQuestionExporterMapper } from '@/features/practiceQuestion/practiceQuestionExporterMapper';
...
export function PracticeQuestionListActions({
  table,
  count,
  filter,
  sorting,
}: {
  filter: z.input<typeof practiceQuestionFilterInputSchema>;
  sorting: DataTableSort;
  count?: number;
  table: Table<PracticeQuestionWithRelationships>;
}) {
  ...
  const isFetching = useIsFetching({ queryKey: ['practiceQuestion'] });
  ...
  const exportMutation = useMutation({
    mutationFn: async () => {
      return await apiClient
        .get(
          `api/practice-question?${objectToQuery({ filter, orderBy: dataTableSortToPrisma(sorting) })}`,
        )
        .json<{
          count: number;
    ...
    onSuccess: (data) => {
      csvExporter(
        practiceQuestionExporterMapper(data.practiceQuestions, {
          dictionary,
          locale,
        }),
        dictionary.practiceQuestion.fields,
        'practiceQuestions',
      );
      toast.success(dictionary.practiceQuestion.export.success);
    },
    onError: (error: any) => {
  ...
  const deleteMutation = useMutation({
    mutationFn: async () => {
      ...
      if (!ids.length) {
        throw new Error(dictionary.practiceQuestion.deleteMany.noSelection);
      }

      return await apiClient
        .delete(`api/practice-question?${objectToQuery({ ids })}`)
        .json();
    },
    onSuccess: () => {
      ...
      toast.success(dictionary.practiceQuestion.deleteMany.success);
    },
    onError: (error: any) => {
      toast.error(error.message || dictionary.shared.errors.unknown);
    },
  ...
  const archiveMutation = useMutation({
    mutationFn: async () => {
      ...
      if (!ids.length) {
        throw new Error(dictionary.practiceQuestion.archiveMany.noSelection);
      }

      return await apiClient
        .put(`api/practice-question/archive`, { json: { ids } })
        .json();
    },
    onSuccess: () => {
      table.resetRowSelection();
      ...
      toast.success(dictionary.practiceQuestion.archiveMany.success);
    },
    onError: (error: any) => {
      toast.error(error.message || dictionary.shared.errors.unknown);
    },
  ...
  const restoreMutation = useMutation({
    mutationFn: async () => {
      ...
      if (!ids.length) {
        throw new Error(dictionary.practiceQuestion.restoreMany.noSelection);
      }

      return await apiClient
        .put(`api/practice-question/restore`, { json: { ids } })
        .json();
    },
    onSuccess: () => {
      table.resetRowSelection();
      ...
      toast.success(dictionary.practiceQuestion.restoreMany.success);
    },
    onError: (error: any) => {
      toast.error(error.message || dictionary.shared.errors.unknown);
    },
  ...
  return (
    <>
      ...
      <Button
        ...
        className="ml-auto flex h-8"
        onClick={() =>
          queryClient.invalidateQueries({ queryKey: ['practiceQuestion'] })
        }
        disabled={!!isFetching}
      >
        {isFetching ? (
      ...
      {hasPermissionToCreate && (
        <Button
          ...
          render={<Link to={`/practice-question/new`} />}
        >
      ...
      {archiveManyDialogOpen && (
        <ConfirmDialog
          title={dictionary.practiceQuestion.archiveMany.confirmTitle}
          description={dictionaryFormat(
            dictionary.practiceQuestion.archiveMany.confirmDescription,
            selectedCount,
          )}
      ...
      {restoreManyDialogOpen && (
        <ConfirmDialog
          title={dictionary.practiceQuestion.restoreMany.confirmTitle}
          description={dictionaryFormat(
            dictionary.practiceQuestion.restoreMany.confirmDescription,
            selectedCount,
          )}
      ...
      {deleteManyDialogOpen && (
        <ConfirmDialog
          title={dictionary.practiceQuestion.deleteMany.confirmTitle}
          description={dictionaryFormat(
            dictionary.practiceQuestion.deleteMany.confirmDescription,
            selectedCount,
          )}
