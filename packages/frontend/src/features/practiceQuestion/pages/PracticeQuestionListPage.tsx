...
import { PracticeQuestionListActions } from '@/features/practiceQuestion/components/PracticeQuestionListActions';
import { PracticeQuestionListFilter } from '@/features/practiceQuestion/components/PracticeQuestionListFilter';
import {
  PracticeQuestionWithRelationships,
  practiceQuestionFilterInputSchema,
...
import { PracticeQuestionNewButton } from '@/features/practiceQuestion/components/PracticeQuestionNewButton';
...
const TABLE_ID = 'practiceQuestion-list';

export const practiceQuestionListLazyRoute = createLazyRoute(
  '/practice-question',
)({
  component: PracticeQuestionListPage,
});

export function PracticeQuestionListPage() {
  ...
  const columns: ColumnDef<PracticeQuestionWithRelationships>[] = [
    {
    ...
    },
    {
      ...
      meta: {
        title: dictionary.practiceQuestion.fields.questionText,
      },
      cell: ({ row }) => (
        <span className="whitespace-nowrap">
          <Link
            className="text-blue-500 hover:text-blue-400 hover:underline focus:text-blue-400 dark:text-blue-400"
            to={`/practice-question/${row?.original?.id}`}
            search={{
              referrer: window.location.pathname + window.location.search,
            }}
          >
            {practiceQuestionLabel(row?.original, dictionary, locale)}
          </Link>
        </span>
      ),
    },
    {
      ...
      meta: {
        title: dictionary.practiceQuestion.fields.difficulty,
      },
      cell: ({ row }) => {
    ...
    },
    {
      ...
      meta: {
        title: dictionary.practiceQuestion.fields.category,
      },
    },
    {
      accessorKey: 'isActive',
      meta: {
        title: dictionary.practiceQuestion.fields.isActive,
      },
      cell: ({ row }) => {
    ...
    },
    {
      accessorKey: 'chapter',
      meta: {
        title: dictionary.practiceQuestion.fields.chapter,
      },
      enableSorting: false,
      cell: ({ row }) => {
    ...
    },
    {
      ...
      meta: {
        title: dictionary.practiceQuestion.fields.createdByMember,
      },
      enableSorting: false,
      cell: ({ row }) => {
    ...
    },
    {
      ...
      meta: {
        title: dictionary.practiceQuestion.fields.createdAt,
      },
      cell: ({ row }) => (
        <span className="whitespace-nowrap">
    ...
    },
    {
      ...
      meta: {
        title: dictionary.practiceQuestion.fields.updatedByMember,
      },
      enableSorting: false,
      cell: ({ row }) => {
    ...
    },
    {
      ...
      meta: {
        title: dictionary.practiceQuestion.fields.updatedAt,
      },
      cell: ({ row }) => (
        <span className="whitespace-nowrap">
    ...
    },
    {
      ...
      cell: ({ row }) => (
        <PracticeQuestionActions mode="table" practiceQuestion={row.original} />
      ),
      enableSorting: false,
      enableHiding: false,
    },
  ...
  const query = useQuery({
    queryKey: ['practiceQuestion', 'list', filter, sorting, pagination],
  ...
  const table = useReactTable({
    ...
    data: query.data?.practiceQuestions || defaultData,
    columns,
  ...
  return (
    <div className="mb-4 flex w-full max-w-full flex-col gap-4 overflow-hidden p-6">
      <div className="flex items-center justify-between">
        <PageHeader items={[[dictionary.practiceQuestion.list.menu]]} />
        {!(
      ...
      {isFilterEmpty(filter) && query.data?.count === 0 && !query.isLoading ? (
        <Empty>
          <EmptyHeader>
            ...
            <EmptyTitle>{dictionary.practiceQuestion.list.title}</EmptyTitle>
            <EmptyDescription>
              {dictionary.practiceQuestion.list.empty}
            </EmptyDescription>
          </EmptyHeader>
          <EmptyContent>
            <div className="flex flex-col items-center gap-6">
              <div className="flex gap-2">
                <PracticeQuestionNewButton />
                {hasPermissionToImport && (
                  <Button
                    nativeButton={false}
                    variant="outline"
                    render={<Link to="/practice-question/importer" />}
                  >
                    {dictionary.practiceQuestion.importer.menu}
                  </Button>
                )}
              </div>
              <Link
        ...
        <>
          <PracticeQuestionListFilter isLoading={query.isLoading} />

          <DataTable
            table={table}
            isLoading={query.isLoading}
            columns={columns}
            notFoundText={dictionary.practiceQuestion.list.noResults}
            newButton={<PracticeQuestionNewButton />}
          />
