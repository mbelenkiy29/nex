import { VerifyEmailRequestWithRecaptcha } from '@/features/auth/components/VerifyEmailRequest';
...
export function VerifyEmailRequestPage() {
  const dictionary = useAuthStore((state) => state.dictionary);
  const config = useAuthStore((state) => state.config);
  const currentOrganization = useAuthStore(
    (state) => state.currentOrganization,
  );
  const search = useSearch({ from: '/auth/verify-email/request' }) as {
    email?: string;
    redirect?: string;
  };
          ...
          <Card className="w-full max-w-md">
            <CardHeader className="text-center">
              <CardTitle className="text-2xl">
                {dictionary.auth.verifyEmailRequest.title}
              </CardTitle>
            </CardHeader>
            <CardContent>
              <p className="mt-4 text-center text-neutral-600 dark:text-neutral-300">
                {dictionary.auth.verifyEmailRequest.noEmail}
              </p>
              <Link
